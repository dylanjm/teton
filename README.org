...
#+begin_example
Directory structure:
├── apps         # Configuration files for applications like: Karabiner, BTT, iTerm, Python, Julia, Rust etc.
...
├── misc         # Temporary location for files that may or may not become a permanant addition to my config
